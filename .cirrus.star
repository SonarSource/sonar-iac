...
def linux_image_template():
    return {
        ...
        'cluster_name': '${CIRRUS_CLUSTER_NAME}', 'region': 'eu-central-1',
        'namespace': 'default', 'use_in_memory_disk': True
    }
...
def linux_1_cpu_2G_template():
    return {
        'eks_container': {
            ...
            'cluster_name': '${CIRRUS_CLUSTER_NAME}', 'region': 'eu-central-1',
            'namespace': 'default',
            'use_in_memory_disk': True, 'cpu': 1, 'memory': '2G'
        }
    }
...
def linux_4_cpu_8G_java_17_template():
    return {
        'eks_container': {
            ...
            'cluster_name': '${CIRRUS_CLUSTER_NAME}', 'region': 'eu-central-1',
            'namespace': 'default',
            'use_in_memory_disk': True, 'cpu': 4, 'memory': '8G'
        }
    }
...
def win_vm_definition():
    return {
        'ec2_instance': {
            'experimental': True, 'image': 'base-windows-jdk17-v*',
            'platform': 'windows', 'region': 'eu-central-1', 'type': 'c5.4xlarge',
            'subnet_id': '${CIRRUS_AWS_SUBNET}', 'preemptible': False,
            'use_ssd': True
        }
    }
...
def qa_task_filter():
    return {
        'only_if': only_sonarsource_qa_filter(),
        'depends_on': ['build']
    }
...
def setup_gradle_cache():
    return {
        'create_gradle_directory_script': ['mkdir -p ${GRADLE_USER_HOME}'],
        'gradle_cache': {
            'folder': '.gradle/caches',
            'fingerprint_script': "echo $(date '+%Y-%m-%d')",
...
def build_template():
    eks_container = {'memory': '6G'}
    eks_container.update(linux_image_with_gcc_template())
    conf = {
...
def build_task_conf():
    # To get a less convoluted conf, a build task factory like for Deep SAST can bring a lot of value
    # Check : https://github.com/SonarSource/deepsast-platform/blob/master/.cirrus/modules/build.star
    conf = {
        'eks_container': {'cpu': 10},
        'env': {
            'DEPLOY_PULL_REQUEST': True,
            'BUILD_ARGUMENTS': '--parallel --profile -x test -x sonar'
        },
    }
    merge_conf_into(conf, build_template())
    merge_conf_into(conf, store_profile_report_template())
...
def qa_os_win_task_conf():
    return {"qa_os_win_task": {
        ...
        'depends_on': ['build'],
        'ec2_instance': {'experimental': True, 'image': 'base-windows-jdk17-v*',
                         'platform': 'windows', 'region': 'eu-central-1', 'type': 'c5.4xlarge',
                         'subnet_id': '${CIRRUS_AWS_SUBNET}', 'preemptible': False,
                         'use_ssd': True},
        'create_gradle_directory_script': ['mkdir -p ${GRADLE_USER_HOME}'],
        'gradle_cache': {'folder': '.gradle/caches',
                         'fingerprint_script': "echo $(date '+%Y-%m-%d')",
                         'reupload_on_changes': 'true'}, 'on_success': {
            'profile_report_artifacts': {'path': 'build/reports/profile/profile-*.html'}},
        '_lib': [
        ...
        'build_script': ['choco install golang --version ${GO_VERSION}',
                         'choco install protoc --version ${PROTOC_VERSION}.0',
                         'eval $(powershell -NonInteractive -Command \'write("export PATH=`"" + ([Environment]::GetEnvironmentVariable("PATH","Machine") + ";" + [Environment]::GetEnvironmentVariable("PATH","User")).replace("\\","/").replace("C:","/c").replace(";",":") + ":`$PATH`"")\')',
                         'source cirrus-env CI', './gradlew ${GRADLE_COMMON_FLAGS} test']}}


# TODO: refactor
def sca_scan_task_conf():
    return {"sca_scan_task": {'create_gradle_directory_script': ['mkdir -p ${GRADLE_USER_HOME}'],
                              'gradle_cache': {'folder': '.gradle/caches',
                                               'fingerprint_script': "echo $(date '+%Y-%m-%d')",
                                               'reupload_on_changes': 'true'},
                              'only_if': '$CIRRUS_USER_COLLABORATOR == \'true\' && $CIRRUS_TAG == "" && ($CIRRUS_BRANCH == $CIRRUS_DEFAULT_BRANCH || $CIRRUS_BRANCH =~ "branch-.*")',
                              ...
                              'eks_container': {'dockerfile': '.cirrus/Dockerfile',
                                                ...
                                                'builder_instance_type': 't3.small',
                                                'builder_subnet_id': '${CIRRUS_AWS_SUBNET}',
                                                'region': 'eu-central-1', 'namespace': 'default',
                                                'use_in_memory_disk': True, 'cpu': 4,
                                                'memory': '8G'},
...
def qa_plugin_task_conf():
    return {"qa_plugin_task": {
        'only_if': '$CIRRUS_USER_COLLABORATOR == \'true\' && $CIRRUS_TAG == "" && ($CIRRUS_PR != "" || $CIRRUS_BRANCH == $CIRRUS_DEFAULT_BRANCH || $CIRRUS_BRANCH =~ "branch-.*" || $CIRRUS_BRANCH =~ "dogfood-on-.*")',
        'depends_on': ['build'], 'eks_container': {
            'image': '${CIRRUS_AWS_ACCOUNT}.dkr.ecr.eu-central-1.amazonaws.com/base:j17-latest',
            'cluster_name': '${CIRRUS_CLUSTER_NAME}', 'region': 'eu-central-1',
            'namespace': 'default', 'use_in_memory_disk': True, 'cpu': 4, 'memory': '8G'},
        'create_gradle_directory_script': ['mkdir -p ${GRADLE_USER_HOME}'],
        'gradle_cache': {'folder': '.gradle/caches',
                         'fingerprint_script': "echo $(date '+%Y-%m-%d')",
                         'reupload_on_changes': 'true'}, 'cleanup_gradle_script': [
            '/usr/bin/find "${CIRRUS_WORKING_DIR}/.gradle/caches" -type d -name "8.*" -prune -maxdepth 1 -exec rm -rf {} \\;',
            'rm -rf "${CIRRUS_WORKING_DIR}/.gradle/caches/journal-1/"'],
        'env': {'GRADLE_TASK': ':its:plugin:integrationTest',
                'matrix': [{'SQ_VERSION': 'LATEST_RELEASE'}, {'SQ_VERSION': 'DEV'}]},
        'run_its_script': ['git submodule update --init --depth 1', 'source cirrus-env QA',
                           'source .cirrus/use-gradle-wrapper.sh',
                           'source set_gradle_build_version $BUILD_NUMBER',
                           './gradlew "${GRADLE_TASK}" "-Dsonar.runtimeVersion=${SQ_VERSION}" "-Dorchestrator.configUrl=https://repox.jfrog.io/repox/orchestrator.properties/orch-h2.properties" "-Dorchestrator.artifactory.accessToken=${ARTIFACTORY_ACCESS_TOKEN}" "-DbuildNumber=$BUILD_NUMBER" --info --build-cache --console plain --no-daemon']}}
...
def qa_ruling_task_conf():
    return {"qa_ruling_task": {
        ...
        'gradle_cache': {'folder': '.gradle/caches',
            ...
            '/usr/bin/find "${CIRRUS_WORKING_DIR}/.gradle/caches" -type d -name "8.*" -prune -maxdepth 1 -exec rm -rf {} \\;',
            'rm -rf "${CIRRUS_WORKING_DIR}/.gradle/caches/journal-1/"'],
        'env': {'GRADLE_TASK': ':its:ruling:integrationTest', 'SQ_VERSION': 'LATEST_RELEASE'},
        'run_its_script': ['git submodule update --init --depth 1', 'source cirrus-env QA',
                           'source .cirrus/use-gradle-wrapper.sh',
                           'source set_gradle_build_version $BUILD_NUMBER',
                           './gradlew "${GRADLE_TASK}" "-Dsonar.runtimeVersion=${SQ_VERSION}" "-Dorchestrator.configUrl=https://repox.jfrog.io/repox/orchestrator.properties/orch-h2.properties" "-Dorchestrator.artifactory.accessToken=${ARTIFACTORY_ACCESS_TOKEN}" "-DbuildNumber=$BUILD_NUMBER" --info --build-cache --console plain --no-daemon']}}
...
def promote_task_conf():
    ...
    merge_conf_into(conf, linux_1_cpu_2G_template())
    return {"promote_task": conf}
