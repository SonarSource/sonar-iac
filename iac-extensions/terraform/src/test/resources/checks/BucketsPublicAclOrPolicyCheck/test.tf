...
resource "aws_s3_bucket" "mynoncompliantfirstbuckets6281default" { # Noncompliant {{Make sure allowing public policy/acl access is safe here.}}
...
resource "aws_s3_bucket_public_access_block" "mynoncompliants6281_publicaccess_1" { # Noncompliant {{Make sure allowing public policy/acl access is safe here.}}
  #      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
resource "aws_s3_bucket_public_access_block" "mynoncompliants6234_missing" { # Noncompliant {{Make sure allowing public policy/acl access is safe here.}}
  #      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  bucket = "mynoncompliantmissingbuckets6234name"
...
resource "aws_s3_bucket_public_access_block" "mynoncompliants6231_multiple_missing" { # Noncompliant {{Make sure allowing public policy/acl access is safe here.}}
  #      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  bucket = "mynoncompliantmultimissingbuckets61234name"
