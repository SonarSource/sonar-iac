...
public class RetrieveRemoteResourcesCheck implements IacCheck {
  ...
  // TODO wget https://example.com/resource -O /path/to/resource

  private static final List<CommandDetector> WGET_DETECTORS = List.of(WGET_DETECTOR);
  ...
  private static boolean containsWgetAuthenticationFlags(List<ArgumentResolution> args) {
    return args.stream().anyMatch(arg ->
      WGET_AUTH_FLAGS.stream().anyMatch(flag -> arg.value().startsWith(flag)));
  }
}
