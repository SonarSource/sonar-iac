...
import org.sonar.iac.docker.plugin.DockerProviders.Provider;
...
class DockerProvidersTest {

  private final AnalysisWarnings analysisWarnings = mock(AnalysisWarnings.class);

  private static final String AWS_KEY = "sonar.docker.provider.aws.version";
  @TempDir
  ...
  void provider_with_valid_version() {
    DockerProviders providers = providers(context(AWS_KEY, "1.3.4"));
  ...
  void provider_with_invalid_version() {
    DockerProviders providers = providers(context(AWS_KEY, "v1.3.4"));
    Provider provider =  providers.provider(Provider.Identifier.AWS);

    assertThat(provider.providerVersion).isNull();
    assertThat(logTester.logs(LoggerLevel.WARN))
      .containsExactly("Can not parse provider version \"sonar.docker.provider.aws.version\". Input: \"v1.3.4\"");
    verify(analysisWarnings, times(1))
      .addUnique("Can not parse provider version for \"sonar.docker.provider.aws.version\". " +
        "Please check the format of your used AWS version in the project settings.");
  }

  @Test
  void provider_with_empty_version() {
    DockerProviders providers = providers(context(AWS_KEY, ""));
    Provider provider =  providers.provider(Provider.Identifier.AWS);
    assertThat(provider.providerVersion).isNull();
    verify(analysisWarnings, times(1))
      .addUnique("Provide the used AWS provider version via the \"sonar.docker.provider.aws.version\" " +
        "property to increase the accuracy of your results.");
  }

  @Test
  void provider_without_provided_version() {
    DockerProviders providers = providers(SensorContextTester.create(baseDir));
  ...
  void check_property_registration() {
    long registeredProperties = DockerSettings.getProperties().stream()
      .filter(propertyDefinition -> propertyDefinition.key().startsWith("sonar.docker.provider"))
      .count();

    assertThat(registeredProperties).isEqualTo(Provider.Identifier.values().length);
  }

  private DockerProviders providers(SensorContext context) {
    return new DockerProviders(context, analysisWarnings);
  }
