...
class VolumeTreeImplTest {
  ...
  void volumeInstructionShell() {
    VolumeTree tree = parse("VOLUME /var/log /var/db", DockerLexicalGrammar.VOLUME);
  ...
  void volumeInstructionEmptyExec() {
    VolumeTree tree = parse("VOLUME []", DockerLexicalGrammar.VOLUME);
    assertThat(tree.arguments().type()).isEqualTo(LiteralListTree.LiteralListType.EXEC);
