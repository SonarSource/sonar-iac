...
RUN openssl genrsa -des3 -passout pass:secret 2048
...
RUN ["sh", "-c", "openssl rsa -pubin -random1 && other command"]
...
RUN ["sh", "-c", "openssl genrsa -des3 -passout pass:x -out server.pass.key 2048 && other command"]
RUN ["sh", "-c", "openssl genrsa -des3 -passout pass:x -out server.pass.key 2048 | other command"]
RUN ["sh", "-c", "openssl genrsa -des3 -passout pass:x -out server.pass.key 2048; other command"]
RUN ["sh", "-c", "openssl genrsa -des3 && other command && other"]
