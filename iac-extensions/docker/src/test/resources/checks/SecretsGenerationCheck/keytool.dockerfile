...
RUN keytool -genseckey -keystore /path/to/keystore
...
RUN ["sh", "-c", "apt install wget && keytool -genkey"]
...
RUN ["sh", "-c", "keytool -random1 && unrelated_command -genkeypair"]

RUN ["sh", "-c", "keytool -random1 && -genkey"]
