...
RUN ["sh", "-c", "curl -L -o output.txt -s https://example.com/resource -k && cat output.txt"]
RUN ["sh", "-c", "sudo curl -L -o output.txt -s https://example.com/resource -k && cat output.txt"]
RUN ["sh", "-c", "command 1; command 2; curl -o php.tar \"$PHP_URL\""]
RUN ["sh", "-c", "curl -o php.tar \"$PHP_URL\"; command 1; command 2"]
RUN ["sh", "-c", "curl -L https://example.com/resource -s -o output.txt -k 1> /dev/null"]
RUN ["sh", "-c", "curl -L -ooutput.txt -s https://example.com/resource -k && cat output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -s --output output.txt -k > /dev/null"]
RUN ["sh", "-c", "curl -O https://example.com/resource > /dev/null"]
RUN ["sh", "-c", "curl -L -k -s https://example.com/resource --remote-name -L -k -s > log.txt"]
RUN ["sh", "-c", "curl -L -k -s --remote-name-all -L -k -s  https://example.com/resource -L -k -s >> log.txt"]

# redirect output ============
RUN ["sh", "-c", "curl https://example.com/resource > output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource > output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource -L > output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k > output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource >> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource >> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource -L >> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k >> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource 1> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource 1> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource -L 1> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k 1> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource 1>> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource 1>> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource -L 1>> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k 1>> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k -ooutput.txt -s >> /dev/null"]
RUN ["sh", "-c", "curl https://example.com/resource -LOv -k 1>> /dev/null"]
...
RUN ["sh", "-c", "if [ -n \"$PHP_URL\" ]; then curl -o php.tar \"$PHP_URL\"; echo \"Success\" fi;"]
...
RUN ["sh", "-c", "curl https://example.com/resource -U user:password > output.txt"]
RUN ["sh", "-c", "curl --tlsuser user -L https://example.com/resource > output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource --tlsuser user >> output.txt"]
RUN ["sh", "-c", "curl -L --proxy-tlspassword password https://example.com/resource >> output.txt"]
RUN ["sh", "-c", "curl https://example.com/resource --proxy-tlspassword password 1> output.txt"]
RUN ["sh", "-c", "curl --tlspassword password -L https://example.com/resource -k 1> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource --tlspassword password 1>> output.txt"]
RUN ["sh", "-c", "curl --proxy-tlspassword password -L https://example.com/resource -k 1>> output.txt"]
RUN ["sh", "-c", "curl -L https://example.com/resource --proxy-tlspassword password -k 1>> output.txt"]
...
RUN ["sh", "-c", "curl https://example.com/resource > /dev/null"]
RUN ["sh", "-c", "curl -L https://example.com/resource >> /dev/null"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k 1> /dev/null"]
RUN ["sh", "-c", "curl -L https://example.com/resource -k 1>> /dev/null"]

# Compliant: curl is part of a command chain and relies on previous commands
RUN ["sh", "-c", "VERSION=$(cat version.txt) && curl -o output.txt https://example.com/resource/$VERSION"]
RUN ["sh", "-c", "VERSION=1.2.3 && curl -o output.txt https://example.com/resource/$VERSION"]
RUN ["sh", "-c", "curl -o output.txt https://example.com/resource/$VERSION"]

ARG RESOURCE_VERSION=1.2.3
RUN ["sh", "-c", "curl -o output.txt https://example.com/resource/$RESOURCE_VERSION"]

# Compliant: '-o-', '--output -' and '-o -' redirect output to stdout and cannot be replaced with ADD, so we don't raise
RUN ["sh", "-c", "curl -o - \"https://raw.githubusercontent.com/nvm-sh/nvm/1.2.3/install.sh\" | bash"]
RUN ["sh", "-c", "curl --output - \"https://raw.githubusercontent.com/nvm-sh/nvm/1.2.3/install.sh\" | bash"]
RUN ["sh", "-c", "curl -o- \"https://raw.githubusercontent.com/nvm-sh/nvm/1.2.3/install.sh\" | bash"]
...
RUN ["sh", "-c", "curl -fSL -o /tmp/example.rpm \"https://example.com/downloads/example_$UNKNOWN.rpm\""]
RUN ["sh", "-c", "export VERSION=$(curl -s https://api.example.com/latest-version | jq -r '.version') && curl -fSL -o /tmp/example.rpm \"https://example.com/downloads/example_$VERSION.rpm\""]
# Sensitive: the unresolved variable is not used in the curl command
RUN ["sh", "-c", "export VERSION=$(curl -s https://api.example.com/latest-version | jq -r '.version') && curl -fSL -o /tmp/example.rpm \"https://example.com/downloads/example_1.2.3.rpm\""]

ARG MY_VERSION=1.2.3

RUN ["sh", "-c", "curl -fSL -o /tmp/example.rpm \"https://example.com/downloads/example_$MY_VERSION.rpm\""]
