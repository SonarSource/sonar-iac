...
RUN ["sh", "-c", "wget -O /path/to/resource https://example.com/resource --limit-rate=100k && wget https://example.com/"]
RUN ["sh", "-c", "sudo wget -O /path/to/resource https://example.com/resource --limit-rate=100k && wget https://example.com/"]
RUN ["sh", "-c", "command 1; command 2; wget -O php.tar \"$PHP_URL\""]
RUN ["sh", "-c", "wget -O php.tar \"$PHP_URL\"; command 1; command 2"]
RUN ["sh", "-c", "wget --no-check-certificate --output-document=/path/to/resource --max-redirect=1 https://example.com/resource --limit-rate=100k | apt-key add - && echo \"success\""]
...
RUN ["sh", "-c", "if [ -n \"$PHP_URL\" ]; then wget -O php.tar \"$PHP_URL\"; echo \"Success\" fi;"]
...
RUN ["sh", "-c", "wget https://example.com/resource > file.html"]

# Compliant: wget is part of a command chain and relies on previous commands
RUN ["sh", "-c", "VERSION=$(cat version.txt) && wget -O output.txt https://example.com/resource/$VERSION"]
RUN ["sh", "-c", "VERSION=1.2.3 && curl -o output.txt https://example.com/resource/$VERSION"]
RUN ["sh", "-c", "wget -O output.txt https://example.com/resource/$VERSION"]

ARG RESOURCE_VERSION=1.2.3
RUN ["sh", "-c", "wget -O output.txt https://example.com/resource/$RESOURCE_VERSION"]

# Compliant: '-O-', '--output-document -' and '-O -' redirect output to stdout and cannot be replaced with ADD, so we don't raise
RUN ["sh", "-c", "wget -O - \"https://example.com/resource\" | bash"]
RUN ["sh", "-c", "wget --output-document - \"https://example.com/resource\" | bash"]
RUN ["sh", "-c", "wget -O- \"https://example.com/resource\" | bash"]
RUN ["wget", "\"http://url1\"", "-O", "output.txt", "\"http://url2\"", "-O", "-"]
RUN ["sh", "-c", "wget -O - https://example.com/resource > download-redirect2.txt"]
RUN ["sh", "-c", "wget --output-document - https://example.com/resource > output.txt"]
...
RUN ["sh", "-c", "export VERSION=$(curl -s https://api.example.com/latest-version | jq -r '.version') && wget -O /tmp/example.rpm \"https://example.com/downloads/example_$VERSION.rpm\""]
# Sensitive: the unresolved variable is not used in the wget command
RUN ["sh", "-c", "export VERSION=$(curl -s https://api.example.com/latest-version | jq -r '.version') && wget -O /tmp/example.rpm \"https://example.com/downloads/example_1.2.3.rpm\""]

ARG MY_VERSION=1.2.3

RUN ["sh", "-c", "wget -O /tmp/example.rpm \"https://example.com/downloads/example_$MY_VERSION.rpm\""]
