...
class KubernetesParserTest {
  ...
  @Test
  void testEmptyParsingWhenHelmContentIsTrue() {
    inputFileContext = new KubernetesInputFileContext(mock(SensorContext.class), inputFile, true);
  ...
  void testNormalParsingWhenNoHelmContent() {
    inputFileContext = new KubernetesInputFileContext(mock(SensorContext.class), inputFile, false);
    FileTree file = parser.parse("foo: {bar: 1234}", inputFileContext);
    assertThat(file.documents()).hasSize(1);
  ...
  @Test
  void testNormalParsingWhenInputFileContextProvided() {
