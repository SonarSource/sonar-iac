...
readonly PROTOBUF_GO_VERSION="${PROTOBUF_GO_VERSION:-1.34.1}"
...
generate_test_report() {
  ...
  path_to_binary=$(install_go "${GO_VERSION}")
  # Test
  CGO_ENABLED=0 bash -c "${path_to_binary} test ./src/... -timeout 1s -coverprofile=build/test-coverage.out -json > build/test-report.json"
}
