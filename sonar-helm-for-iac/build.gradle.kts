...
val artifactFilesField = ExecutableLocator::class.java.getDeclaredField("artifactFiles").also { it.isAccessible = true }
val protocPathProvider = providers.provider { artifactFilesField.get(tools.protoc) as FileCollection }.map {
    // As long as we don't use any protobuf plugins, the first file should be a discovered protoc executable.
    it.files.first()
}.map {
...
if (isCi) {
    val compileProtobufGoTask = tasks.register<Exec>("compileProtobufGo") {
        ...
        val dummyGeneratedFile = project.layout.buildDirectory.file("generated.pb.txt")
        outputs.files(dummyGeneratedFile)
        outputs.cacheIf { true }

        commandLine(
            protocPathProvider.get(), "-I=${project.projectDir}/proto", "-I=${System.getProperty("user.home")}/go/protobuf/include", "-I=/opt/protoc/include",
            "--go_out=${project.projectDir}/src", "template-evaluation.proto", "ast.proto"
        )

        doLast {
            dummyGeneratedFile.get().asFile.writeText("These files are generated by the build process.")
        }
    }
